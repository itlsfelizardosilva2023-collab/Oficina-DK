...
require_once ".\models\Atualizacao_carro_models.php";
session_start();
?>
...
<html lang="pt">
 <head>
    ...
    <link rel="stylesheet" href="../views/css/atualizacao_carro.css?v=1.0">
  </head>
   <body>
...
<select name="id_marca" id="marca" onchange="carregarModelos(this.value)">
    ...
    while($m = $res->fetch_assoc()){
        ...
        // 🔥 marca atual selecionada
        $selected = ($m['id_marca'] == $carro['id_marca']) ? "selected" : "";

        echo "<option value='{$m['id_marca']}' $selected>{$m['nome']}</option>";
    }
    ?>
...
<select name="id_modelo" id="modelo">
    <?php
    // 🔥 buscar modelos da marca atual
    $res_modelos = $conn->query("
        SELECT * FROM modelos 
        WHERE id_marca = ".$carro['id_marca']
    );

    while($mod = $res_modelos->fetch_assoc()){

        // 🔥 modelo atual selecionado
        $selected = ($mod['id_modelo'] == $carro['id_modelo']) ? "selected" : "";

        echo "<option value='{$mod['id_modelo']}' $selected>{$mod['nome']}</option>";
    }
    ?>
</select>
    ...
    <!-- Outros dados -->
   <input type="text" name="matricula" placeholder="Matrícula Exe:. LD-00-00-XX"
value="<?= htmlspecialchars($_POST['matricula'] ?? $carro['matricula']) ?>">

<?php if(!empty($erros['matricula'])): ?>
...
value="<?= htmlspecialchars($_POST['cor'] ?? $carro['cor']) ?>">
...
<label><br>Cliente: </br>  </label>
<select name="id_cliente" required>
    <option value="">Selecione um cliente</option>
    <?php
    foreach($clientes as $id => $nome){
        $selected = ($id == $carro['id_cliente']) ? "selected" : "";
    ...
    <a href="carros.php"><h3>Voltar</h3></a>
</form>
